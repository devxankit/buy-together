  ...
  return (
    <div className="flex flex-col min-h-[100dvh] w-full max-w-[430px] mx-auto bg-[#FAFAFA] font-sans">
      <div className="flex items-center gap-3 px-5 pt-5 pb-4 bg-surface border-b border-line sticky top-0 z-20">
        ...
        <h1 className="text-[15px] font-black text-ink">Notification Preferences</h1>
      </div>

      <div className="flex-1 px-5 py-5 flex flex-col gap-5 pb-10">
        {sections.map(sec => (
          <div key={sec.title}>
            <h3 className="text-[12px] font-black text-faint uppercase tracking-wide mb-3">{sec.title}</h3>
            <div className="bg-surface border border-line rounded-2xl overflow-hidden">
              {sec.items.map((item, idx) => (
                <div key={item.key} className={`flex items-center justify-between px-4 py-3.5 ${idx !== sec.items.length - 1 ? 'border-b border-line' : ''}`}>
