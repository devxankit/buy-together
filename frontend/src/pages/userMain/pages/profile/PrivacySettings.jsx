  ...
  return (
    <div className="flex flex-col min-h-[100dvh] w-full max-w-[430px] mx-auto bg-[#FAFAFA] font-sans">
      <div className="flex items-center gap-3 px-5 pt-5 pb-4 bg-white border-b border-slate-100 sticky top-0 z-20">
        ...
        <h1 className="text-[15px] font-black text-slate-800">Privacy Settings</h1>
      </div>

      <div className="flex-1 px-5 py-5">
        <div className="bg-white border border-slate-100 rounded-2xl overflow-hidden">
          {items.map((item, idx) => (
            <div key={item.key} className={`flex items-center justify-between px-4 py-3.5 ${idx !== items.length - 1 ? 'border-b border-slate-50' : ''}`}>
