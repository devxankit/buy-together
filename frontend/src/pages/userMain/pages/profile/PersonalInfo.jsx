      ...
      <div className="flex-1 px-5 py-6 flex flex-col gap-4">
        ...
        <div className="flex flex-col items-center mb-2">
          <div className="relative">
            <img src="https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?auto=format&fit=crop&w=150&q=80" alt="Avatar" className="w-20 h-20 rounded-full object-cover border-2 border-surface shadow-md" />
        ...
        <div className="border-2 border-line rounded-2xl px-4 py-3 bg-surface">
          ...
          <div className="flex gap-2">
            {['Male', 'Female', 'Other'].map(g => (
              <button key={g} onClick={() => setGender(g)} className={`flex-1 py-2 rounded-xl text-[12px] font-bold transition-all active:scale-95 ${gender === g ? 'bg-primary text-white shadow-sm' : 'bg-surface-alt text-faint'}`}>{g}</button>
            ))}
          </div>
        </div>

        <button className="w-full h-[50px] bg-primary rounded-2xl text-white text-[14px] font-black flex items-center justify-center gap-2 shadow-lg shadow-teal-500/25 active:scale-95 transition-all mt-2">
          Save Changes
        </button>
      </div>
    </div>
